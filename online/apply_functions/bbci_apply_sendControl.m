function data_feedback= bbci_apply_sendControl(control_signal, bbci_feedback, data_feedback, state)
...
%             info = lsl_streaminfo(lib,'BBCI','BCI',1,100,'cf_float32','BBCIonlinetoolbox');
            info = lsl_streaminfo(lib,['Feedback'],'CTRL',5,10,'cf_float32','sdfwerr32432');
...
switch(bbci_feedback.receiver),
    ...
    case 'lsl',
        if ~isempty(control_signal),
            data_feedback.opt.outlet.push_sample([control_signal{2}; ...
                state.acq.mrkDesc(state.acq.trgIdx-1)]);
        end
    case 'matlab',
